...
function fail(res, status, code, message, fields = undefined) {
...
app.get("/api/domains", (req, res) => {
  const domains = db.prepare("SELECT DISTINCT domain FROM internships ORDER BY domain").all()
    .map(row => row.domain);
  ok(res, domains);
});
...
app.get("/api/internships", (req, res) => {
  ...
  if (search) {
    conditions.push(`(
      title LIKE @search OR company LIKE @search OR
      location LIKE @search OR description LIKE @search OR skills LIKE @search
    )`);
  ...
  return ok(res, rows.map(serialize), 200, {
...
app.post("/api/internships", (req, res) => {
  ...
  const info = statement.run({
    ...result.value,
    skills: JSON.stringify(result.value.skills)
  });

  const row = db.prepare("SELECT * FROM internships WHERE id = ?").get(info.lastInsertRowid);
...
app.put("/api/internships/:id", (req, res) => {
  ...
  if (!existing) return fail(res, 404, "NOT_FOUND", "Internship not found.");
  ...
  db.prepare(`
    UPDATE internships SET
      title=@title, company=@company, location=@location, domain=@domain,
      type=@type, duration=@duration, stipend=@stipend, description=@description,
      skills=@skills, apply_url=@apply_url, updated_at=CURRENT_TIMESTAMP
    WHERE id=@id
  `).run({
    ...result.value,
    id,
...
app.post("/api/applications", (req, res) => {
  ...
  const internship = db.prepare("SELECT id FROM internships WHERE id = ?").get(result.value.internship_id);
  if (!internship) return fail(res, 404, "INTERNSHIP_NOT_FOUND", "That internship no longer exists.");

  try {
    const info = db.prepare(`
      INSERT INTO applications
      (internship_id, name, email, phone, resume_url, cover_note)
...
if (require.main === module) {
  app.listen(PORT, () => {
    console.log(`Internship Portal running at http://localhost:${PORT}`);
  });
}
